...
#include "./mouse/analog.h"
#include "./mouse/usb_mouse_debug.h"

#include "./mouse/print.h"
...
int main(void) {
	...
	int16_t delx, dely;
	  // DO THE MOUSE
	  dely = adc_read(MOUSE_Y);
	  delx = adc_read(MOUSE_X);
		mx = (int8_t)( delx >> 4 ) - 0x20;
		my = (int8_t)( dely >> 4 ) - 0x20;
		mx = mx <= 3 && mx >= -3 ? 0 : mx;
		my = my <= 3 && my >= -3 ? 0 : my;
		print("read x=");
		phex16(delx);
